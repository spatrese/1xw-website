...
def safe_str(x: Any) -> str: return "" if x is None else str(x)
...
def canonical_asset_class(ac: str) -> str:
    ...
    return {"equity":"Equities","equities":"Equities","rates":"Rates","fx":"FX","commodities":"Commodities","commodity":"Commodities","crypto":"Crypto"}.get(s, safe_str(ac).strip() or "Other")
...
SETUP_BONUS_LONG = {"breakout":0.10,"trend continuation":0.05,"trend continuation (down)":-0.05,"neutral":0.0,"mean reversion (bounce)":-0.03,"mean reversion (pullback)":-0.03,"breakdown":-0.05}
SETUP_BONUS_SHORT = {"breakdown":0.10,"trend continuation (down)":0.05,"trend continuation":0.05,"neutral":0.0,"mean reversion (bounce)":-0.03,"mean reversion (pullback)":-0.03,"breakout":-0.05}
...
POS_WORDS = ["rally","gain","surge","rebound","beat","upgrade","cuts","cut","easing","dovish","disinflation","soft landing","stimulus","expansion","strong demand","risk-on","optimism","upside","growth"]
NEG_WORDS = ["selloff","drop","plunge","crash","miss","downgrade","hikes","tightening","hawkish","recession","slowdown","war","conflict","risk-off","warning","stress","shock","downside"]
...
def build_fund_commentary(ac: str, tone: str, bias: float, conf: float, top_news: List[Dict[str, Any]]) -> str:
    ...
    }.get(ac,{}).get(tone, f"{ac} tone is {tone.lower()} this week.")
...
def build_fundamentals(news_digest: Dict[str, Any], per_class_news: int = 3) -> Dict[str, Any]:
    by = news_digest.get("by_asset_class"); ac_map: Dict[str,List[Dict[str,Any]]] = {}
    if isinstance(by, dict):
        for ac, lst in by.items():
            if isinstance(lst, list): ac_map[canonical_asset_class(ac)] = lst
    out = {"by_asset_class": {}}
    for ac, lst in ac_map.items():
        ...
        if scores:
            s_sum = sum(scores); bias = clamp(s_sum/max(8.0,2.5*len(scores)), -1.0, 1.0); mean = s_sum/len(scores)
    ...
    if isinstance(by_symbol, dict):
        ...
        for sym, row in by_symbol.items():
            if isinstance(row, dict):
                rr = dict(row); rr.setdefault("symbol", sym); out.append(rr)
        return out
    return []
def build_technical_overview(universe_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for r in universe_rows:
        ...
        by_symbol[row["symbol"]] = row; by_ac.setdefault(ac, []).append(row)
    by_asset_class = {}
    for ac, rows in by_ac.items():
        scores = [parse_float(rr.get("score")) for rr in rows if parse_float(rr.get("score")) is not None]
        bullish, bearish = sum(1 for s in scores if s >= 2), sum(1 for s in scores if s <= -2)
...
def map_event_to_asset_classes(ev: Dict[str, Any]) -> List[str]:
    tags = ev.get("markets") or ev.get("asset_tags") or []; out = []
    if isinstance(tags, list):
        for t in tags:
            ct = canonical_asset_class(t)
            if ct not in out: out.append(ct)
    return out
def short_event_label(title: str, ev_type: str) -> str:
...
def attach_events_to_fundamentals(fundamental_overview: Dict[str, Any], event_calendar: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for ev in event_calendar:
        item = {"label":short_event_label(safe_str(ev.get("title")), safe_str(ev.get("type"))),"title":safe_str(ev.get("title")),"source":safe_str(ev.get("source")),"date":safe_str(ev.get("date")),"url":safe_str(ev.get("url"))}
...
def build_rationale(side: str, setup: str, asset_class: str, tone: str) -> str:
    s = setup_key(setup)
    tech_text = "Breakout setup" if "breakout" in s else "Trend strength" if side=="LONG" and "trend continuation" in s else "Breakdown setup" if "breakdown" in s else "Downtrend remains in place" if side=="SHORT" and "down" in s else "Constructive technical setup" if side=="LONG" else "Fragile technical setup"
...
def build_top_ideas(universe_rows: List[Dict[str, Any]], fund_overview: Dict[str, Any], n_top: int = 3) -> Dict[str, Any]:
    ...
    for r in universe_rows:
        ...
        if not sym: continue
        ac = canonical_asset_class(r.get("asset_class") or r.get("assetClass") or "Other")
        setup = safe_str(r.get("setup") or ""); score = parse_float(r.get("score")) or 0.0
        tlong, tshort = tech_scores(r)
        f = fund_by.get(ac, {}); bias = parse_float(f.get("bias")) or 0.0; conf = parse_float(f.get("confidence")) or 0.0; tone = safe_str(f.get("tone") or "Mixed")
        fund = bias * conf; f_long, f_short = clamp((fund+1.0)/2.0), clamp((-fund+1.0)/2.0)
...
def main():
    ...
    screener = read_json(args.screener); news_digest = read_json(args.news) if os.path.exists(args.news) else {}
    ...
    event_calendar = build_event_calendar(start_date=asof_d, end_date=asof_d + timedelta(days=7))
